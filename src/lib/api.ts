...
const API_BASE = ""; // same-origin; Next.js Route Handlers proxy to the FastAPI backend
...
export async function runBatch(files: File[]): Promise<{ success: boolean; results: InferenceResult[] }> {
  ...
  const r = await fetch(`/api/inference/batch`, { method: "POST", body: fd });
  if (!r.ok) throw new Error(`Batch inference failed: ${r.status}`);
  return r.json();
}

export async function fetchHistory(): Promise<{ entries: HistoryEntry[] }> {
  const r = await fetch(`/api/inference-history`, { cache: "no-store" });
  return r.json();
}

export async function deleteHistory(id: string): Promise<void> {
  await fetch(`/api/inference-history/${encodeURIComponent(id)}`, { method: "DELETE" });
}
...
export async function analyzeRestoration(file: File, derainedFile?: File): Promise<any> {
  ...
  const r = await fetch(`/api/analyze/restoration`, { method: "POST", body: fd });
...
export async function analyzeXray(file: File, derainedFile?: File): Promise<any> {
  ...
  fd.append("file", file);
  if (derainedFile) fd.append("derained_file", derainedFile);
  const r = await fetch(`/api/analyze/xray`, { method: "POST", body: fd });
...
export async function analyzeObjects(file: File, derainedFile?: File): Promise<any> {
  ...
  fd.append("file", file);
  if (derainedFile) fd.append("derained_file", derainedFile);
  const r = await fetch(`/api/analyze/objects`, { method: "POST", body: fd });
...
export async function askVisionAssistant(question: string, file: File, derainedFile?: File): Promise<any> {
  ...
  const r = await fetch(`/api/vision-assistant`, { method: "POST", body: fd });
...
export function blendUrl(originalName: string, derainedName: string, alpha: number): string {
  return `/api/blend?original_id=${encodeURIComponent(originalName)}&derained_id=${encodeURIComponent(derainedName)}&alpha=${alpha}`;
}
